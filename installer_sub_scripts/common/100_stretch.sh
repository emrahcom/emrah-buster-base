...
echo STRETCH="$IP" >> $INSTALLER/000_source
...
then
    echo STRETCH_SKIPPED=true >> $INSTALLER/000_source
...
lxc-create -n $MACH -t download -P /var/lib/lxc/ -- \
    ...
    --server "uk.images.linuxcontainers.org" \
    --keyserver "keyserver.ubuntu.com"

# shared directories
mkdir -p $SHARED/cache
cp -arp $MACHINE_HOST/usr/local/eb/cache/stretch_apt_archives $SHARED/cache/
